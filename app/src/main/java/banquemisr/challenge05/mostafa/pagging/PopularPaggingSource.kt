...
class PopularPaggingSource(private val remoteDataSource: IRemoteDataSource) : PagingSource<Long, Results>() {
    @RequiresExtension(extension = Build.VERSION_CODES.S, version = 7)
    override suspend fun load(params: LoadParams<Long>): LoadResult<Long, Results> {
        return try {
            val currentPage = params.key ?: 1L
            val movies = remoteDataSource.getPopular(currentPage)
            LoadResult.Page(
                data = movies.first().results,
                prevKey = if (currentPage == 1L) null else currentPage - 1L,
                nextKey = if (movies.first().results.isEmpty()) null else movies.first().page!! + 1L
            )
